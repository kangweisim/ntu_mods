...
const LoginForm = props => {
  ...
  const handleSubmit = event => {
    ...
    errorCatcher(async () => {
      ...
      const { entity, access_token } = await API.Auth.login({ username, password });
      if (mounted) {
        setShowSuccess(true);
        if (entity && entity.person)
          setFirstname(entity.person.firstname);
        else if (entity && entity.organisation)
          setFirstname(entity.organisation.name);
        else
          setFirstname(null);
      }

      setTimeout(() => {
        dispatch(ProfileActions.update_session(entity, access_token));
      }, 1000);
  ...
  if (showSuccess) {
    return (
      <Box display="flex" flexDirection="column" alignItems="center" className={classes.success}>
        ...
        <Typography color="inherit" variant="h4">Welcome back, {firstname || "user"}.</Typography>
        <Typography color="inherit" variant="caption">Login Success</Typography>
      </Box>
    )
  }
